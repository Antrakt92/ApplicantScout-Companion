...
2. Confirm the paired addon release is current. Companion `0.5.2` pairs with
...
3. Run:
   ...
   .\scripts\check.ps1
   .\scripts\check-release-version.ps1 -Tag v0.5.2
   ```
...
2. Optional local smoke: build the installer and portable archive before the tag
   ...
   .\scripts\check-release-version.ps1 -Tag v0.5.2 -RequireAssets
   ```

3. Expected assets:
   - `dist\ApplicantScoutCompanionSetup-0.5.2.exe`
   - `dist\ApplicantScoutCompanionSetup-0.5.2.exe.sha256`
   - `dist\ApplicantScoutCompanion-0.5.2-portable.zip`

## Publish

1. Push tag `v0.5.2` after release-prep changes are committed.
...
5. Smoke-test from an installed `0.5.1` companion: update check should show the
   ...
   silently, close the old process, and relaunch `0.5.2`.
