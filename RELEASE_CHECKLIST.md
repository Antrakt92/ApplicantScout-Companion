...
2. Confirm the paired addon release is current. Companion `0.5.5` pairs with
   ApplicantScout addon `0.3.4`.
3. Run:

   ```powershell
   .\.venv\Scripts\python -m pytest
   .\scripts\check.ps1
   .\scripts\check-release-version.ps1 -Tag v0.5.5
   ```
...
2. Optional local smoke: build the installer and portable archive before the tag
   ...
   .\scripts\check-release-version.ps1 -Tag v0.5.5 -RequireAssets
   ```

3. Expected assets:
   - `dist\ApplicantScoutCompanionSetup-0.5.5.exe`
   - `dist\ApplicantScoutCompanionSetup-0.5.5.exe.sha256`
   - `dist\ApplicantScoutCompanion-0.5.5-portable.zip`

## Publish

1. Push tag `v0.5.5` after release-prep changes are committed.
...
5. Smoke-test from an installed `0.5.1` companion: update check should show the
   ...
   silently, close the old process, and relaunch `0.5.5`.
