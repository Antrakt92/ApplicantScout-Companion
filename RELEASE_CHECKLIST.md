...
2. Confirm the paired addon release is current. Companion `0.2.4` pairs with
   ApplicantScout addon `0.1.6`.
3. Run:

   ```powershell
   .\.venv\Scripts\python -m pytest
   .\scripts\check.ps1
   .\scripts\check-release-version.ps1 -Tag v0.2.4
   ```
...
2. Optional local smoke: build the installer and portable archive before the tag
   ...
   .\scripts\check-release-version.ps1 -Tag v0.2.4 -RequireAssets
   ```

3. Expected assets:
   - `dist\ApplicantScoutCompanionSetup-0.2.4.exe`
   - `dist\ApplicantScoutCompanionSetup-0.2.4.exe.sha256`
   - `dist\ApplicantScoutCompanion-0.2.4-portable.zip`
...
5. Smoke-test from an installed `0.2.3` companion: update check should show the
   ...
   silently, close the old process, and relaunch `0.2.4`.
