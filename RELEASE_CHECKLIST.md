...
2. Confirm the paired addon release train is prepared. Companion `0.7.1` pairs
   with ApplicantScout addon `0.4.2`; the addon `CHANGELOG.md` top entry must
   name Companion `0.7.1`.
3. Run:

   ```powershell
   .\.venv\Scripts\python -m pytest
   .\scripts\check.ps1
   .\scripts\check-release-version.ps1 -Tag v0.7.1
   ```
...
2. Optional local smoke: build the installer and portable archive before the tag
   ...
   .\scripts\check-release-version.ps1 -Tag v0.7.1 -RequireAssets
   ```

3. Expected assets:
   - `dist\ApplicantScoutCompanionSetup-0.7.1.exe`
   - `dist\ApplicantScoutCompanionSetup-0.7.1.exe.sha256`
   - `dist\ApplicantScoutCompanion-0.7.1-portable.zip`
...
1. Push the paired addon/companion tags close together after release-prep
   ...
   `ApplicantScout-v0.4.2.zip` and `release.json` before it creates the
   companion draft.
...
6. Smoke-test from an installed `0.5.1` companion: update check should show the
   ...
   silently, close the old process, and relaunch `0.7.1`.
